...
export function computeStats(trades) {
  ...
  const gs = ts => ({
    ...
    win_rate: ts.length ? (ts.filter(t => t.pnl > 0).length / ts.length) : 0 // Returns decimal 0-1
  })
  ...
  // Monthly grouping - ensure date is valid
  const monthG = grpBy(trades.filter(t => t.entry_time), t => {
    try {
  ...
  // Daily grouping for cumulative
  const dateG = grpBy(trades.filter(t => t.entry_time), t => {
    try {
      const d = new Date(t.entry_time)
      if (isNaN(d.getTime())) return 'Unknown'
  ...
  sorted.forEach(t => {
    cum += t.pnl || 0
    try {
      const dateKey = new Date(t.entry_time).toISOString().slice(0, 10)
      cumByDate[dateKey] = cum
    } catch {}
  })
  ...
  return {
    overview: {
      ...
      win_rate: wins.length / (trades.length || 1), // Decimal 0-1
    ...
    monthly: Object.entries(monthG).map(([month_str, ts]) => ({
      month: month_str, // Changed from month_str to month for chart compatibility
      ...gs(ts)
