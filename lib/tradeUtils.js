...
import { parseBybitSpot, isBybitSpot }     from './parsers/bybit-spot'
...
export function parseTradeFile(rows, filename, accountIdOverride = null) {
  ...
  // Detect Hyperliquid
  if (isHyperliquid(rows)) return parseHyperliquid(rows, filename, accountIdOverride || 'HL_DEFAULT')
...
export function computeStats(trades) {
  ...
  trades.forEach(t => {
    const ts = t.exit_time || t.entry_time  // use exit_time for realised P&L date
    if (!ts) return
  ...
  sorted.forEach(t => {
    const ts = t.entry_time || t.exit_time
    if (!ts) return
    cum += t.pnl; cumByDate[ts.slice(0, 10)] = cum
  })
  ...
  // Annualised P&L = total_pnl × (365 / calendar days in period)
  const totalPnl = trades.reduce((s, t) => s + t.pnl, 0)
  let calmar = null
  if (maxDrawdown > 0 && sorted.length >= 20) {
    const firstTs = sorted[0].entry_time  || sorted[0].exit_time
    const lastTs  = sorted[sorted.length-1].entry_time || sorted[sorted.length-1].exit_time
    if (firstTs && lastTs) {
