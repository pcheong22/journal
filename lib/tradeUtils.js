...
export function parseTradeFile(rows, filename, accountIdOverride = null) {
  ...
  const parsePrimeXBTDate = (dateStr) => {
    ...
    // Format: "01/04/2025 14:39" or "13/12/2024 08:48"
  ...
  if (broker === 'PrimeXBT') {
    trades = rows.slice(hRow + 1)
      .filter(r => r[4]?.trim().toUpperCase() === 'EXECUTED')
      .map(r => {
        const entryTime = parsePrimeXBTDate(r[2]) // Column 2 is Entry time
        return {
          position_id: `PXT_${r[0]}_${Date.parse(entryTime)}`,
          entry_time: entryTime,
          exit_time: parsePrimeXBTDate(r[3]), // Column 3 is Exit time
