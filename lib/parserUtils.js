...
// ── NOTIONAL USD CALCULATOR (with FX conversion) ──────────────────────────────
export function calculateNotionalUSD(size, price, symbol = '', currency = 'USD') {
  if (!size || !price || price <= 0) return null
  
  let notional = size * price
  
  // Approximate FX rates to convert non-USD base currencies to USD
  // (Update these periodically or replace with a live API if needed)
  const FX_TO_USD = { 
    'EUR': 1.085, 'GBP': 1.27, 'JPY': 0.00665, 'CHF': 1.13, 
    'AUD': 0.65, 'CAD': 0.735, 'NZD': 0.60, 'HKD': 0.128 
  }
  
  if (currency !== 'USD' && FX_TO_USD[currency]) {
    notional *= FX_TO_USD[currency]
  }
  
  // Special handling for JPY-quoted pairs (e.g., USD/JPY, EUR/JPY)
  // Price is in JPY per unit, so notional in USD ≈ size / price
  if (currency === 'JPY' || symbol?.includes('JPY')) {
    notional = size / price
  }
