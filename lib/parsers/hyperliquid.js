...
export function parseHyperliquid(rows, filename, accountId) {
  ...
  for (let i=0; i<closeEvents.length; i++) {
    ...
    const ev = closeEvents[i]
    const related = closeEvents.filter((e,j) =>
      !usedEvtIdxs.has(j) && e.coin===ev.coin && e.dir===ev.dir &&
      Math.abs(e.time-ev.time) <= CONSOLIDATE_MS
    )
    related.forEach((_,j) => usedEvtIdxs.add(closeEvents.indexOf(related[j])))
    // re-find indices properly
    const group = []
    for (let j=i; j<closeEvents.length; j++) {
      if (!usedEvtIdxs.has(j) && closeEvents[j].coin===ev.coin &&
          closeEvents[j].dir===ev.dir &&
          Math.abs(closeEvents[j].time-ev.time) <= CONSOLIDATE_MS) {
        group.push(closeEvents[j]); usedEvtIdxs.add(j)
      }
    }
    if (!usedEvtIdxs.has(i)) { group.push(ev); usedEvtIdxs.add(i) }
    tradeGroups.push(group.length ? group : [ev])
  }
  ...
  tradeGroups.forEach((group, gi) => {
    ...
    for (let i=opens.length-1; i>=0; i--) {
      ...
      if (!usedOpenIdxs.has(i) && o.coin===coin &&
          openSides.includes(o.dir) && o.time<=group[0].time) {
        bestOpen=o; usedOpenIdxs.add(i); break
      }
    }
