...
export function parseHyperliquid(rows, filename, accountId) {
  ...
  // Merge opens and close events per coin+dir, sort by time,
  // track net position — when it hits 0 the trade is complete

  const tradeGroups = []

  // Build all events per coin+dir
  ...
  Object.entries(allEventsByCoinDir).forEach(([key, events]) => {
    ...
    // Partial position still open — emit what we have
    if (curCloses.length > 0) {
      tradeGroups.push({ opens:[...curOpens], closes:[...curCloses] })
    }
  })
  ...
  tradeGroups.forEach((group, gi) => {
    const { opens: gOpens, closes: gCloses } = group
    ...
    const entryPrice   = totalOpenSz > 0 ? wEntryPx/totalOpenSz : null
    const entryTime    = gOpens.length > 0 ? gOpens[0].time : null
