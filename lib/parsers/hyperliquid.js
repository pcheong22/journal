...
function normaliseHLSymbol(coin) {
  ...
  if (mapped) return applySymbolRules(normaliseSymbol(mapped))
  const cleaned = coin.replace(/\s*\(.*?\)/g, '').replace(/^[a-z]+:/, '').trim()
  return applySymbolRules(normaliseSymbol(cleaned))
}

// Normalise symbol suffixes and ensure bare coin names get /USD
// Rules: /USDT → /USD, /USDC → /USD, bare symbol (no /) → SYMBOL/USD
// Exception: symbols with (xyz) suffix keep their qualifier
function applySymbolRules(symbol) {
  if (!symbol) return symbol
  // Replace /USDT and /USDC with /USD
  symbol = symbol.replace(/\/USDT$/i, '/USD').replace(/\/USDC$/i, '/USD')
  // If no slash and no parenthesis qualifier, append /USD
...
// Convert a local-time ISO string to UTC using a given IANA timezone.
function localToUTC(localIso, tz) {
  ...
  const tzAsUtc = Date.UTC(+p.year, +p.month - 1, +p.day,
    p.hour === '24' ? 0 : +p.hour, +p.minute, +p.second)
  const offsetMs = tzAsUtc - utcRef.getTime()
  return new Date(new Date(localIso + 'Z').getTime() - offsetMs)
}

function parseHLDate(s, tz) {
  s = String(s || '').trim()
  // Native format: user's browser local time — apply timezone offset to get UTC.
  // tz is an IANA timezone string passed from Settings (e.g. 'Asia/Dubai').
  // Falls back to treating as UTC if tz not provided.
  const native = s.match(/^(\d{2})\/(\d{2})\/(\d{4})\s*-\s*(\d{2}:\d{2}:\d{2})/)
  if (native) {
    const localIso = `${native[3]}-${native[2]}-${native[1]}T${native[4]}`
    if (tz && tz !== 'UTC') return localToUTC(localIso, tz)
    return new Date(localIso + 'Z')
  }
  // ISO format (third-party) — already UTC
...
export function parseHyperliquid(rows, filename, accountId, timezone) {
  const currency = 'USDC'
  ...
  dataRows.forEach(r => {
    ...
    const time = parseHLDate(r[iTime], timezone)
  ...
  // Orphan fills on the same UTC calendar day = same trade (partial closes of one position)
  // Use 24 hours so intraday orphan closes always merge, but next-day ones stay separate.
  ...
  // ── Step 2b: Merge orphan fragments back into their completed trade ────────
  // When the proportional split creates an orphan from the same close event
  // as a completed trade (same coin + exit time + exit px), merge it in.
  // This prevents a single close order appearing as two separate line items.
  const mergedGroups = []
  tradeGroups.forEach(group => {
    if (!group.orphan) { mergedGroups.push(group); return }
    // Try to find a completed trade with same coin, exit time (within 1s), same exit px
    const orphanExitMs = group.closes[0]?.time?.getTime()
    const orphanPx     = group.closes[0]?.px
    const orphanCoin   = group.closes[0]?.coin
    const match = mergedGroups.find(g => {
      if (g.orphan) return false
      const gExitMs = g.closes[g.closes.length-1]?.time?.getTime()
      const gPx     = g.closes[g.closes.length-1]?.px
      const gCoin   = g.closes[0]?.coin
      return gCoin === orphanCoin &&
             Math.abs((gExitMs||0) - (orphanExitMs||0)) <= 1000 &&
             Math.abs((gPx||0)     - (orphanPx||0))     < 0.001
    })
    if (match) {
      // Absorb orphan closes into the matched completed trade
      match.closes.push(...group.closes)
    } else {
