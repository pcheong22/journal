...
export function parseHyperliquid(rows, filename, accountId) {
  ...
  // Group all close events for same coin+direction until cumulative closed size
  // equals cumulative opened size (position fully flat)
  closeEvents.sort((a, b) => a.time - b.time)
  opens.sort((a, b) => a.time - b.time)

  // Build per-coin open size timeline for position tracking
  // openedByCoin[coin][dir] = sorted list of {time, sz}
  const openedByCoin = {}
  opens.forEach(o => {
    const dir = o.dir === 'Open Long' || o.dir === 'Buy' ? 'Long' : 'Short'
    const key = `${o.coin}|${dir}`
    if (!openedByCoin[key]) openedByCoin[key] = []
    openedByCoin[key].push(o)
  })

  // Track cumulative open size per coin+dir up to a given time
  const openSizeUpTo = (coin, dir, beforeTime) => {
    const key = `${coin}|${dir}`
    const arr = openedByCoin[key] || []
    return arr.filter(o => o.time <= beforeTime).reduce((s, o) => s + (o.sz || 0), 0)
  }
  ...
  closeEvents.forEach((ev, i) => {
    ...
    if (!coinDirGroups[key]) coinDirGroups[key] = []
    coinDirGroups[key].push({ ev, i })
  })

  Object.values(coinDirGroups).forEach(items => {
    items.sort((a, b) => a.ev.time - b.ev.time)
    let i = 0
    while (i < items.length) {
      if (usedEvtIdxs.has(items[i].i)) { i++; continue }

      const group     = []
      let closedSz    = 0
      const coin      = items[i].ev.coin
      const dir       = directionFromClose(items[i].ev.dir, items[i].ev.pnl)
      const firstTime = items[i].ev.time

      // Total opened size up to (and including) the first close
      let targetSz = openSizeUpTo(coin, dir, firstTime)
      if (targetSz === 0) targetSz = Infinity // unknown — collect all

      // Collect ALL close events for this position until fully flat
      // A new position starts only when there's a new open AFTER some closes
      for (let j = i; j < items.length; j++) {
        if (usedEvtIdxs.has(items[j].i)) continue
        group.push(items[j])
        usedEvtIdxs.add(items[j].i)
        closedSz += items[j].ev.size

        // Check if there are new opens between this close and the next close
        // If yes and we've closed most of the position, this is a new trade
        if (j + 1 < items.length) {
          const nextCloseTime = items[j+1].ev.time
          const newOpens = (openedByCoin[`${coin}|${dir}`] || [])
            .filter(o => o.time > items[j].ev.time && o.time < nextCloseTime)
          const nextOpenSz = newOpens.reduce((s,o) => s+(o.sz||0), 0)
          // If significant new opens between closes, this position is done
          if (nextOpenSz > closedSz * 0.05) break
        }
      }

      if (group.length > 0) tradeGroups.push(group.map(g => g.ev))
      i += group.length || 1
    }
  })
