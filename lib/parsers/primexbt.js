...
import { getSession, getTvSymbol, normaliseSymbol, parseDate } from '../parserUtils'
...
export function parsePrimeXBT(rows, accountId, currency = 'USDC') {
  ...
  for (const close of closing) {
    ...
    const notional     = amount && exitPrice ? Math.round(amount * exitPrice * 100) / 100 : null
    ...
    trades.push({
      ...
      pnl:           Math.round(close.pnlRaw * 100) / 100,
      pct_gain:      pctGain,
      fee,
