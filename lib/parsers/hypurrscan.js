...
function normaliseHSSymbol(token) {
  ...
  // "HYPE-USD" → "HYPE/USD", "xyz:SILVER-USD" → "SILVER/USD", "XLM-USD" → "XLM/USD"
  const cleaned = token
    ...
    .trim()
  const base = normaliseSymbol(cleaned)
  // Apply unified rules: bare symbol → SYMBOL/USD, /USDT → /USD, /USDC → /USD
  return applySymbolRules(base)
}

// Shared symbol normalisation rules (mirrors hyperliquid.js applySymbolRules)
