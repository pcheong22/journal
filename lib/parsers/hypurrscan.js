...
function isTwap(type)  { return type === 'Twap' }
...
export function parseHypurrscan(rows, filename, accountId) {
  ...
  if (headerIdx < 0) return { broker: 'Hyperliquid', accountId, currency, trades: [], upsertMode: true }
  ...
  // Only process PERP rows (class === 'PERP') and TWAP rows
  // Skip WITHDRAW, SPOT, TRANSFER etc.
  const fills = []
  dataRows.forEach(r => {
    const cls  = String(r[iClass] || '').trim()
    const type = String(r[iType]  || '').trim()
    if (cls !== 'PERP' && !isTwap(type)) return
    if (!isOpen(type) && !isClose(type) && !isTwap(type)) return
    ...
    fills.push({
      ...
      sign:   amount >= 0 ? 1 : -1, // +1 = buy/open, -1 = sell/close
      px:     parseNum(r[iPx]),
  ...
  // ── Group TWAP fills into events ────────────────────────────────────────
  // TWAP rows represent the cumulative fill of a TWAP order.
  // A positive amount = opening (buy), negative = closing (sell).
  // Pair consecutive TWAP open+close events for the same token within 30 min.
  const TWAP_WINDOW_MS = 30 * 60 * 1000

  const twapFills  = fills.filter(f => isTwap(f.type))
  const otherFills = fills.filter(f => !isTwap(f.type))

  // Group TWAP fills by token, pair open+close
  const twapTrades = []
  const twapByToken = {}
  twapFills.forEach(f => {
    if (!twapByToken[f.token]) twapByToken[f.token] = []
    twapByToken[f.token].push(f)
  })

  Object.entries(twapByToken).forEach(([token, tFills]) => {
    tFills.sort((a, b) => a.time - b.time)
    let pendingOpen = null
    tFills.forEach(f => {
      if (f.sign > 0) {
        // Positive = opening a long position via TWAP
        pendingOpen = f
      } else if (f.sign < 0 && pendingOpen) {
        // Negative = closing — check if within window of last open
        const gap = f.time - pendingOpen.time
        if (gap <= TWAP_WINDOW_MS) {
          twapTrades.push({ open: pendingOpen, close: f })
          pendingOpen = null
        }
      }
    })
  ...
  // ── Group regular close fills into close events (sequential, 2-min gap) ─
  ...
  // ── Add TWAP trades ──────────────────────────────────────────────────────
  twapTrades.forEach(({ open, close }, i) => {
    const symbol     = normaliseHSSymbol(open.token)
    const entryTime  = open.time
    const exitTime   = close.time
    const entryPrice = open.px
    const exitPrice  = close.px
    const dir        = 'Long' // TWAP buys then sells (long round-trip)
    const sz         = Math.min(open.amount, close.amount)
    const durationMins = Math.max(0, (exitTime - entryTime) / 60000)
    const hour       = exitTime.getUTCHours()
    const fee        = Math.round((open.fee + close.fee) * 100) / 100
    const positionId = `HL_TWAP_${String(open.token).replace(/\W/g, '')}_${exitTime.getTime()}_${i}`

    const { notional_usd, method: notional_method } = calculateNotionalUSD(
      null, entryPrice, exitPrice, sz, dir
    )
    ...
    trades.push({
      ...
      symbol,
      direction:     dir,
      size:          Math.round(sz * 1e6) / 1e6,
      entry_price:   entryPrice ? Math.round(entryPrice * 1e6) / 1e6 : null,
      exit_price:    exitPrice  ? Math.round(exitPrice  * 1e4) / 1e4 : null,
      notional_usd,
      notional_method,
      pnl:           null, // not in Hypurrscan — estimated by upload API if possible
      fee,
