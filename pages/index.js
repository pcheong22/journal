...
const SESSION_TIMES = { 
  Asia:'23:00–07:00 GMT', 
  London:'07:00–12:00 GMT', 
  'London/NY Overlap':'12:00–16:00 GMT', 
  'New York':'16:00–22:00 GMT' 
}
const SESSION_CLASS = { 
  Asia:'pill-asia', 
  London:'pill-london', 
  'London/NY Overlap':'pill-overlap', 
  'New York':'pill-ny' 
}
...
export default function Dashboard() {
  ...
  const handleClear = async (accountId = null) => {
    const msg = accountId 
      ...
      : 'Delete ALL data for ALL accounts?'
  ...
  const renderCalendar = () => {
    ...
    for (let i=0; i < shift; i++) {
      cells.push(<div key={`e${i}`} className="cal-cell emp" />)
    }
    ...
    for (let d=1; d <= days; d++) {
      ...
      if (info) { 
        mPnl += info.total_pnl
        mTrades += info.count
        mWins += info.win_rate * info.count
        mDays++ 
      }
      cells.push(
      ...
      <>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginTop:12}}>
          {[
            ...
            ['TRADING DAYS', mDays, 'var(--tx)'],
            ['TRADES', mTrades, 'var(--tx)'],
            ['WIN RATE', mwr, 'var(--ac)']
          ].map(([l,v,c]) => (
        ...
        <div style={{background:'var(--sf)',borderBottom:'1px solid var(--bd)',padding:'8px 24px',display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
          ...
          <button 
            ...
            style={{padding:'4px 10px',borderRadius:5,border:'1px solid',fontSize:11,fontFamily:'var(--font-mono)',cursor:'pointer',fontWeight:600,transition:'all .15s',
              background:selAccounts.size === 0 ? 'var(--ac)' : 'var(--sf2)',
              borderColor:selAccounts.size === 0 ? 'var(--ac)' : 'var(--bd)',
              color:selAccounts.size === 0 ? '#fff' : 'var(--tx2)'}}
          >
...
<ChartComp key={privacy ? 'privacy-on' : 'privacy-off'} type="equity" data={stats.cumulative} privacy={privacy} /> 
        ...
        {tab === 'calendar' && (
          <div className="anim">
            <div className="card">
              {stats ? renderCalendar() : <div style={{color:'var(--mu)',textAlign:'center',padding:20}}>Loading…</div>}
            </div>
          </div>
        )}
        ...
        {tab === 'symbols' && stats && (
          <div className="anim">
            ...
            <div className="card">
              <div className="ct">
                <span className="ind" />SYMBOL BREAKDOWN
              ...
              <div className="tw">
                <table style={{minWidth:540}}>
                  <thead>
                    <tr>
                      {['Symbol','Trades','Total P&L','Win Rate','Avg P&L','Assessment'].map(h => <th key={h}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {stats.symbols.map(s => {
                      const wr = Math.round(s.win_rate * 100)
                      const badge = s.total_pnl > 10000 && s.win_rate > .65 
                        ? <span className="pill pb">KEEP</span> 
                        : s.total_pnl < -10000 
                          ? <span className="pill pr">REDUCE</span> 
                          : <span className="pill pn">MONITOR</span>
                      return (
                        <tr key={s.symbol}>
                          <td className="sym-c">{s.symbol}</td>
                          <td>{s.count}</td>
                          <td className={`${s.total_pnl >= 0 ? 'pos' : 'neg'} private`}>{fU(Math.round(s.total_pnl))}</td>
                          <td>
                            <div style={{display:'flex',alignItems:'center',gap:6}}>
                              <div style={{width:48,background:'var(--sf3)',borderRadius:2,height:4}}>
                                <div style={{width:wr+'%',height:4,borderRadius:2,background:wr >= 65 ? 'var(--wn)' : wr < 50 ? 'var(--ls)' : 'var(--wa)'}} />
                              </div>
                              <span className={wr >= 65 ? 'pos' : wr < 50 ? 'neg' : 'wa'}>{wr}%</span>
                            </div>
                          </td>
              ...
              <>
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:8,marginBottom:12}}>
                  {stats.sessions.filter(s => s.session !== 'Other').map(s => {
                    ...
                    const wrc = s.win_rate >= .65 ? 'var(--wn)' : s.win_rate >= .55 ? 'var(--wa)' : 'var(--ls)'
                    return (
                      <div key={s.session} className="sc">
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
                          ...
                          <span className={`pill ${s.win_rate >= .65 ? 'pb' : s.win_rate >= .55 ? 'pw' : 'pr'}`}>
                            {s.win_rate >= .65 ? '▲ EDGE' : s.win_rate >= .55 ? '→ OK' : '▼ WEAK'}
                          </span>
                        </div>
                ...
                <div className="card">
                  ...
                  <div className="tw">
                    <table style={{minWidth:420}}>
                      <thead>
                        <tr>
                          {['Day','Trades','Total P&L','Win Rate','Avg P&L','Signal'].map(h => <th key={h}>{h}</th>)}
                        </tr>
                      </thead>
                      <tbody>
                        {stats.daily_dow.map(d => (
                          <tr key={d.day_of_week}>
                            <td className="sym-c">{d.day_of_week}</td>
                            <td>{d.count}</td>
                            ...
                            <td>
                              <span className={`pill ${d.win_rate >= .65 ? 'pb' : d.win_rate < .55 ? 'pr' : 'pw'}`}>
                                {d.win_rate >= .65 ? '▲ TRADE FULL' : d.win_rate < .55 ? '▼ AVOID' : '→ HALF SIZE'}
                              </span>
                            </td>
                          </tr>
                        ))}
        ...
        {tab === 'trades' && (
          <div className="anim">
            <div style={{fontSize:11,color:'var(--mu)',marginBottom:10,fontFamily:'var(--font-mono)'}}>
              Click any row to open · ← → keys to navigate · notes and tags saved per trade
            </div>
            <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:10,flexWrap:'wrap',background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:8,padding:'10px 14px',boxShadow:'var(--sh-sm)'}}>
              ...
              {[
                [fSym, setFSym, ['All Symbols', ...symbols]],
                [fDir, setFDir, ['Long & Short', 'Long', 'Short']],
                [fRes, setFRes, ['Win & Loss', 'win', 'loss']],
                [fSess, setFSess, ['All Sessions', 'Asia', 'London', 'London/NY Overlap', 'New York']]
                  ...
                  <thead>
                    <tr>
                      {[
                        ...
                        <th key={k} className={sortKey === k ? 'th-s' : ''} onClick={() => {
                          if (!k.startsWith('_')) {
                            setSortKey(k)
                    ...
                    {paged.map((t, i) => {
                      ...
                      return (
                        <tr key={t.id || i} className="clk" onClick={() => setSelected(t)}>
                          ...
                          <td>
                            {acc && (
                              <span style={{display:'inline-flex',alignItems:'center',gap:4,padding:'2px 6px',borderRadius:4,background:acc.color + '15',border:`1px solid ${acc.color}30`,fontSize:10,fontFamily:'var(--font-mono)',fontWeight:600}}>
                                <span style={{width:5,height:5,borderRadius:'50%',background:acc.color}} />
                                {acc.label || acc.id}
                              </span>
                            )}
                          </td>
                          <td>
                            <span className={`pill ${t.direction === 'Long' ? 'pb' : 'pr'}`}>
  ...
  return (
    <div className="mo" onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:10,padding:24,width:340,boxShadow:'var(--sh-lg)',margin:'auto'}}>
        ...
        <input 
          className="inp" 
          style={{width:'100%',marginBottom:14,padding:'8px 12px',fontSize:13}} 
          value={label}
          onChange={e => setLabel(e.target.value)} 
          onKeyDown={e => e.key === 'Enter' && onSave(account.id, label)} 
          autoFocus 
        />
...
<div class="footer">Trading Journal · AI Coaching Report · ${report.period || periodLabel}</div>
  ...
  const displayInsights = report?.insights 
    ? (typeof report.insights === 'string' ? JSON.parse(report.insights) : report.insights) 
    : []
  ...
  return (
    <div className="anim">
      ...
      {showHist && (
        <div className="card" style={{marginBottom:14}}>
          <div className="ct">
            <span className="ind" />SAVED REPORTS {histLoad && <span style={{fontSize:10,fontWeight:400,color:'var(--mu)'}}>Loading…</span>}
          </div>
